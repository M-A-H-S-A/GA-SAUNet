...
import recall_score, roc_auc_score, accuracy_score, confusion_matriximport  ModelCheckpointfrom util
import numpy as np
...
for y in y_pred:
    ...
    y = y * 255
    cv2.imwrite(path_experiment+'png' % i, y)
    i+=1
...
print('NPV:', tn / (tn + fn))
print("JS:", jaccard_similarity_score(y_test, list(np.ravel(y_pred))))
print('PPV', tp / (tp + fp))
...
file_perf.write("Area under the ROC curve: "+str(roc_auc_score)
                + "\nJaccard similarity score: " +str(jaccard_similarity_score)
